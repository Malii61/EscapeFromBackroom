...
namespace StarterAssets
{
    ...
    public class PlayerController : MonoBehaviourPunCallbacks
    {
        ...
        private void Update()
        {
            if (!PV.IsMine || isPlayerAtLobby || CanvasManager.IsOpen())
            {
                if (_input)
                {
        ...
        private void HandleInteractions()
        {
            ...
            if (Physics.Raycast(ray, out RaycastHit raycastHit, INTERACT_ALTERNATE_DISTANCE))
            {
                ...
                {
                    float distance = Vector3.Distance(ray.origin, raycastHit.transform.position);
                    if (distance > INTERACTION_DISTANCE)
                    {
                        if (raycastHit.transform.TryGetComponent(out TextRevealer revealer))
                            // interact alternate object has found
                            Interact(revealer);
                    }
                    else
                    {
